...
func TestLexicalTag(t *testing.T) {
	...
	if !hasTag(tokens[0].Tags, chunky.TagDET) {
		t.Errorf("tokens[0] ('the') candidates = %v, want DET", tokens[0].Tags)
	}
}
...
func TestLexicalTagPreservesCompounds(t *testing.T) {
	...
	if !hasTag(tokens[0].Tags, chunky.TagADP) {
		t.Errorf("'such as' candidates = %v, want ADP", tokens[0].Tags)
	}
}
