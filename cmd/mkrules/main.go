...
//
// All-pairs mode (globally sorted, single var):
//
//	{ F1=NOUN F2=VERB bash rules.sh | sed 's/^/NOUN|VERB|/'; ... } | go run ./cmd/mkrules -allpairs
package main
...
// parseSlots parses a feature+key pair into rule slot values, updating r.
// Returns false and logs to stderr on error.
func parseSlots(r *rule, feature, key string) bool {
...
func emitRules(w *bufio.Writer, varName string, rules []rule) {
...
func main() {
	allPairsFlag := flag.Bool("allpairs", false, "read TAG1|TAG2|-prefixed 7-field lines; emit globally-sorted generatedRules var")
	...
	flag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "       { ... | sed 's/^/T1|T2|/' } | mkrules -allpairs [-pkg PKG]")
		flag.PrintDefaults()
	}
	flag.Parse()
	...
	emitRules(w, *varFlag, rules)
}

func runAllPairs(pkg string) {
	var rules []rule
	scanner := bufio.NewScanner(os.Stdin)
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text())
		...
		parts := strings.Split(line, "|")
		// 7 fields: tag1|tag2|<word>|feature|key|resolve|<count>
		if len(parts) != 7 {
			fmt.Fprintf(os.Stderr, "skipping malformed line: %q\n", line)
			continue
		}
		tag1, err := chunky.ParseTag(parts[0])
		if err != nil {
			fmt.Fprintf(os.Stderr, "bad tag1 %q: %v\n", parts[0], err)
			continue
		}
		tag2, err := chunky.ParseTag(parts[1])
		if err != nil {
			fmt.Fprintf(os.Stderr, "bad tag2 %q: %v\n", parts[1], err)
			continue
		}
		feature := parts[3]
		key := parts[4]
	...
	fmt.Fprintf(w, "// Code generated by cmd/mkrules. DO NOT EDIT.\n")
	fmt.Fprintf(w, "package %s\n\n", pkg)
	fmt.Fprintf(w, "// generatedRules is the globally-sorted context disambiguation rule table.\n")
	fmt.Fprintf(w, "// Sorted most-specific-first (most active context slots first).\n")
	fmt.Fprintf(w, "// %d rules consolidated from %d (%d merged).\n", after, before, before-after)
	emitRules(w, "generatedRules", rules)
}
